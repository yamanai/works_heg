<template>
	<div class="wrapper">
		<head-top class="header">
			...
			<img slot="right" class="placeholder" :src="require('../../../assets/images/flight-prev.png')">
		</head-top>
		<mt-popup v-model="showPopup" popup-transition="popup-fade" modal="true" closeOnClickModal="false">
			<failure>
				<div slot="head">Age is wrong</div>
				<p v-for="(item,index) in errorMassages" slot="content">
					<strong>{{index+1}}</strong>.{{item}}</p>
