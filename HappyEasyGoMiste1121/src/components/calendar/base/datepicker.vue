<template>
    <div class="calendar-input-container" v-show="show" :class="[isRed ? 'red-theme' : 'blue-theme']" :style="containerStyle">
    
        <label for="calendar-input">
    
            <input ref="mainInput" v-show="false" class="calendar-input" type="text" readonly="readonly" :class="{focus: isfocus}" @click="focus" v-model="selectValue">
        </label>
        ...
        <transition name="fade">
            <div id="calendar" v-show="changeShowCalendar">
                <div id="calendar-header">
                    ...
                    <span class="arrow" :class="{disabled:selectMonth==limit.minMonth}" style="fontSize:80%" @click="subMonth">&lt;</span>
                    <span id="date-box">
                        {{trueSelectMonths}} {{trueSelectYear}}
                    </span>
                    </span>
                    <span class="arrow" :class="{disabled:selectMonth==limit.maxMonth}" style="fontSize:80%" @click="addMonth">&gt;</span>
                    <!-- <span class="arrow" :class="{disabled:selectYear==limit.maxYear}" @click="addYear">&gt;&gt;</span> -->
                </div>
                <div class="week">
                    <span v-for="(item, index) in week" :key="item.id" :class="{weekend: index === 0 || index === 6}">
                        {{item}}
                    </span>
                </div>
                <div class="days">
                    <span v-for="(item, index) in renderData" :key="item.id" :class="{
                                    weekend: index % 7 === 0 || index % 7 === 6,
                                    unselect: unselectArr.includes(index),
                                    select: index === firstDayInWeek + trueSelectDay - 1}" @click="changeSelectDay(index)">
                        {{item}}
                    </span>
