		...
		<div class="home-wrapper" v-show="!showSearch" @touchstart="touchstart" @touchmove="touchend">
			<div class="home-content">
				...
				<head-top class="header ">
					...
					<span slot="right" class="text" @click="$router.push('/offer')">Offers</span>
				</head-top>
				<div class="trip-control">
					<div class="trip-control-btn flex space-between align-items-center">
						<div class="btn-wrap" :class="{active:isOne}">
							<a id="oneway" @click="showOne">One way</a>
						</div>
						<div class="btn-wrap" :class="{active:isRound}">
				...
				<div class="content">
					<div class="content-main">
						...
						<div class="offer-content">
							<img src="../../../assets/images/homeoffer/guaranteed1.png" alt="" @click="$router.push('/guaranteed')">
						</div>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		loadSearchInfo() {
			if (sessionStorage.searchInfo) {
				...
				this.departAirport = getAirpotByAitaCode(info.departAirport.iataCode);
				this.arriveAirport = getAirpotByAitaCode(info.arriveAirport.iataCode);
	...
	mounted() {
		...
		if (sessionStorage.searchInfo) {
			...
			this.returnDate = new Date(new Date());
			this.departAirport = getAirpotByAitaCode('DEL');
			this.arriveAirport = getAirpotByAitaCode('BOM');
		}
		let cu = new CookieUtil(document)
